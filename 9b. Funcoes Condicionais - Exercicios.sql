...
DECLARE @regular FLOAT = 0.07
DECLARE @deluxe FLOAT = 0.07
SELECT
...
/*2. Você ficou responsável pelo controle de produtos da empresa e deverá fazer uma análise da quantidade de produtos por Marca. A divisão das marcas em categorias deverá ser a seguinte: 
...
SELECT 
	...
	CASE
		...
		WHEN EmployeeCount >= 10 THEN 'Entre 10 e 40 Funcionários'
		ELSE 'Abaixo de 10 Funcionários'
	END AS 'Categoria'
FROM DimStore


/*4. O setor de logística deverá realizar um transporte de carga dos produtos que estão no depósito de Seattle para o depósito de Sunnyside. Não se tem muitas informações sobre os produtos que estão no depósito, apenas se sabe que existem 100 exemplares de cada Subcategoria. Ou seja, 100 laptops, 100 câmeras digitais, 100 ventiladores, e assim vai. 
...
SELECT
	...
	CASE
		WHEN ROUND(AVG(Weight)*100,2) >=1000 THEN 'Rota2'
		ELSE 'Rota1'
	END AS 'Rota'
FROM DimProduct
...
/* 5. O setor de marketing está com algumas ideias de ações para alavancar as vendas em 2021. Uma delas consiste em realizar sorteios entre os clientes da empresa. Este sorteio será dividido em categorias: 
